...
export default function ArtikelDetailPage() {
  ...
  useEffect(() => {
    async function fetchArtikel() {
      try {
        const res = await fetch(`${typeof window === "undefined" ? "http://127.0.0.1:8000/api" : "/api"}/artikel/${slug}`);
        if (!res.ok) {
          if (res.status === 404) {
        ...
        // Fetch related artikel
        try {
          const resRelated = await fetch(`${typeof window === "undefined" ? "http://127.0.0.1:8000/api" : "/api"}/artikel/${slug}/related`);
          if (resRelated.ok) {
  ...
  if (error || !artikel) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        <h2 className="text-2xl font-bold text-gray-800 mb-4">{error || "Artikel tidak ditemukan"}</h2>
  ...
  const shareTitle = artikel ? encodeURIComponent(artikel.judul) : '';
      ...
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 md:pt-12">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap items-center text-sm font-medium text-gray-500 gap-y-1 min-w-0">
            ...
            <Link href="/artikel" className="hover:text-[#7FC248] transition-colors shrink-0">Artikel</Link>
            <span className="mx-1 sm:mx-2 shrink-0">/</span>
            <span className="text-gray-900 truncate max-w-[120px] sm:max-w-[200px] md:max-w-xs">{artikel.judul}</span>
          </div>
        </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between py-4 border-y border-gray-100 gap-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#7FC248] flex items-center justify-center text-white font-bold text-lg">
                {artikel.user?.name 
                  ? artikel.user.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()
                  : "TZ"}
              </div>
              <div>
                <p className="text-sm font-semibold text-gray-900">{artikel.user?.name || "Redaksi Taman Zakat"}</p>
          ...
          <div className="mt-12 pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex flex-wrap gap-2">
              {artikel.tags ? (
                ...
                <span className="px-4 py-1.5 bg-gray-100 text-gray-600 text-sm font-medium rounded-full hover:bg-gray-200 cursor-pointer transition-colors">#ArtikelTerkini</span>
              )}
            </div>
            ...
            <div className="relative">
              ...
              {showShareMenu && (
                <div className="absolute bottom-full mb-3 right-0 bg-white border border-gray-100 rounded-2xl shadow-xl p-2.5 flex flex-col gap-1 w-56 z-10 transition-all origin-bottom-right">
                  ...
                  <a href={`https://api.whatsapp.com/send?text=Baca artikel menarik ini: ${shareTitle} - ${shareUrl}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 hover:bg-[#25D366]/10 hover:text-[#25D366] rounded-xl text-sm font-semibold text-gray-700 transition-colors group">
                    <WhatsappIcon />
                    WhatsApp
                  </a>
                  ...
                  <a href={`https://twitter.com/intent/tweet?url=${shareUrl}&text=Baca artikel menarik ini: ${shareTitle}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-100 hover:text-gray-900 rounded-xl text-sm font-semibold text-gray-700 transition-colors group">
                    <TwitterIcon />
                    Twitter / X
                  </a>
      ...
      {related.length > 0 && (
        <section className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 pt-12 border-t-2 border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {related.map((item) => (
              <Link key={item.id} href={`/artikel/${item.slug}`} className="group flex flex-col sm:flex-row gap-4 bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition-all">
