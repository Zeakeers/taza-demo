...
<div class="space-y-8">
    ...
    <div class="flex items-start justify-between gap-4">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl lg:text-3xl font-bold text-dark">Edit Formulir</h1>
                <p class="text-gray-400 text-[10px] lg:text-base mt-1">Perbarui formulir "{{ $customForm->title }}".</p>
            </div>
        </div>
    </div>

    <form action="{{ route('admin.custom-forms.update', $customForm) }}" method="POST" id="form-builder">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-[2.5rem] p-6 sm:p-8 lg:p-10 shadow-xl border border-white mb-8">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $customForm->title) }}" required placeholder="Contoh: Formulir Pendaftaran Kegiatan" class="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-primary transition-all outline-none text-base">
                </div>
                <div>
                    <label class="block font-bold mb-2 text-sm">Deskripsi (Opsional)</label>
                    <textarea name="description" rows="3" placeholder="Deskripsikan form ini secara singkat..." class="w-full px-6 py-4 rounded-2xl bg-gray-50 border border-gray-100 focus:border-primary transition-all outline-none text-sm">{{ old('description', $customForm->description) }}</textarea>
                </div>
                {{-- Share Link (read-only) --}}
                <div>
                    <label class="block font-bold mb-2 text-sm text-gray-400">Link Formulir (Tidak bisa diubah)</label>
                    <div class="flex items-center gap-2">
                        <input type="text" value="{{ route('public.form.show', $customForm->slug) }}" readonly class="flex-1 px-6 py-4 rounded-2xl bg-gray-100 border border-gray-100 outline-none text-sm text-gray-500" id="edit-link">
                        <button type="button" onclick="copyEditLink()" class="bg-primary/10 text-primary hover:bg-primary hover:text-white p-4 rounded-2xl transition-all" title="Salin Link">
        ...
        <div class="bg-white rounded-[2.5rem] p-6 sm:p-8 lg:p-10 shadow-xl border border-white mb-8">
            ...
            <div id="fields-container" class="space-y-4">
                @foreach($customForm->fields as $idx => $field)
                    <div class="field-item bg-gray-50 border border-gray-100 rounded-2xl p-5 relative group" data-index="{{ $idx }}">
                            ...
                            <div class="flex-1 space-y-4">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" name="fields[{{ $idx }}][label]" value="{{ $field->label }}" required placeholder="Contoh: Nama Lengkap" class="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-primary transition-all outline-none text-sm">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-bold text-gray-500 mb-1.5">Tipe Input</label>
                                        <select name="fields[{{ $idx }}][type]" onchange="toggleOptions(this)" class="hidden" data-custom-select>
                                            ...
                                            <option value="radio" {{ $field->type === 'radio' ? 'selected' : '' }}>Pilihan Ganda (Radio)</option>
                                            <option value="checkbox" {{ $field->type === 'checkbox' ? 'selected' : '' }}>Centang (Checkbox)</option>
                                            <option value="file" {{ $field->type === 'file' ? 'selected' : '' }}>Upload File</option>
                                        </select>
                                        <div class="custom-select-wrapper" data-for="fields[{{ $idx }}][type]">
                                            <div class="custom-select-trigger" onclick="toggleCustomSelect(this)">
                                                <span class="custom-select-text">{{ ['text'=>'Teks Singkat','textarea'=>'Teks Panjang','email'=>'Email','number'=>'Angka','date'=>'Tanggal','select'=>'Dropdown Pilihan','radio'=>'Pilihan Ganda (Radio)','checkbox'=>'Centang (Checkbox)','file'=>'Upload File'][$field->type] ?? $field->type }}</span>
                                ...
                                <div class="options-wrapper {{ in_array($field->type, ['select', 'radio', 'checkbox']) ? '' : 'hidden' }}">
                                    ...
                                    <input type="text" name="fields[{{ $idx }}][options]" value="{{ $field->options ? implode(', ', $field->options) : '' }}" placeholder="Opsi 1, Opsi 2, Opsi 3" class="w-full px-4 py-3 rounded-xl bg-white border border-gray-200 focus:border-primary transition-all outline-none text-sm">
                                </div>
                                <div class="flex items-center gap-3">
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="fields[{{ $idx }}][is_required]" class="sr-only peer" {{ $field->is_required ? 'checked' : '' }}>
...
<script>
    let fieldCounter = {{ $customForm->fields->count() }};
    ...
    function toggleCustomSelect(trigger) {
        ...
        if (!isOpen) { options.classList.remove('hidden'); trigger.classList.add('active'); }
    }
...
<style>
    ...
    .custom-select-trigger { display:flex; align-items:center; justify-content:space-between; width:100%; padding:0.75rem 1rem; border-radius:0.75rem; background:#fff; border:1px solid #e5e7eb; cursor:pointer; transition:border-color 0.2s,box-shadow 0.2s; font-size:0.875rem; color:#1f2937; }
    .custom-select-trigger:hover { border-color:#5DA630; }
    .custom-select-trigger.active { border-color:#5DA630; box-shadow:0 0 0 3px rgba(93,166,48,0.12); }
    .custom-select-arrow { width:18px; height:18px; color:#9ca3af; transition:transform 0.2s; flex-shrink:0; }
    .custom-select-trigger.active .custom-select-arrow { transform:rotate(180deg); }
    .custom-select-options { position:absolute; top:calc(100% + 6px); left:0; right:0; background:#fff; border:1px solid #e5e7eb; border-radius:1rem; box-shadow:0 12px 36px rgba(0,0,0,0.12); z-index:50; overflow:hidden; animation:dropdownOpen 0.2s ease-out; max-height:280px; overflow-y:auto; }
    ...
    .custom-select-option { display:flex; align-items:center; gap:0.75rem; padding:0.7rem 1rem; cursor:pointer; font-size:0.875rem; color:#374151; transition:background 0.15s; }
    .custom-select-option:hover { background:#f0fdf4; }
    .custom-select-option.selected { background:#f0fdf4; color:#166534; font-weight:600; }
</style>
@endsection
