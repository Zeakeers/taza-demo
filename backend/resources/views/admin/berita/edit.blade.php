...
@section('content')
    ...
    <form action="{{ route('admin.berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
                ...
                <div class="space-y-6">
                    <div>
                        <label class="block text-sm font-bold text-zinc-700 mb-2">Judul Berita <span
                    ...
                    <div>
                        <label class="block text-sm font-bold text-zinc-700 mb-2">Kategori <span
                                class="text-red-500">*</span></label>
                        <input type="hidden" name="kategori" id="kategori-input" value="{{ old('kategori', $berita->kategori) }}" required>
                    ...
                    <div>
                        ...
                        <input type="text" name="tags" value="{{ old('tags', $berita->tags) }}"
                            placeholder="Contoh: TamanZakat, BeritaTerkini, Sosial (pisahkan dengan koma)"
                            class="w-full bg-gray-50 border border-gray-200 text-zinc-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-zinc-700 mb-2">Isi Konten Berita <span
                                class="text-red-500">*</span></label>
                        <textarea id="konten-editor" name="konten" rows="12" placeholder="Tuliskan isi berita di sini..."
                            class="w-full bg-gray-50 border border-gray-200 text-zinc-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all resize-y">{{ old('konten', $berita->konten) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <label for="file-upload"
                            class="mt-1 flex flex-col items-center justify-center border-2 border-gray-300 border-dashed rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors cursor-pointer relative overflow-hidden group w-full">
                            <div class="space-y-1 text-center w-full py-6 {{ $berita->thumbnail ? 'hidden' : '' }}"
                            ...
                            <img id="image-preview"
                                ...
                                class="w-full object-contain {{ $berita->thumbnail ? '' : 'hidden' }}" />

                            <!-- Overlay untuk ganti gambar -->
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none {{ $berita->thumbnail ? '' : 'hidden' }}"
                                id="hover-overlay">
                            ...
                            <input id="file-upload-overlay" name="thumbnail_overlay" type="file"
                                class="absolute inset-0 w-full h-full opacity-0 cursor-pointer {{ $berita->thumbnail ? '' : 'hidden' }}"
                                accept="image/*"
                    ...
                    <div class="space-y-4">
                        <label
                            ...
                            <input type="checkbox" name="show_on_home" value="1" {{ old('show_on_home', $berita->show_on_home) ? 'checked' : '' }}
                        ...
                        <label
                            class="flex items-center gap-3 p-3 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors">
                            <input type="checkbox" name="is_popular" value="1" {{ old('is_popular', $berita->is_popular) ? 'checked' : '' }}
                                class="w-5 h-5 text-yellow-500 focus:ring-yellow-500 rounded border-gray-300">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-zinc-800">Berita Populer</span>
                                <span class="text-xs text-gray-500">Akan masuk ke section Berita Populer</span>
                            </div>
                        </label>
                        ...
                        <label
                            ...
                            <input type="checkbox" name="is_published" value="1" {{ old('is_published', $berita->is_published) ? 'checked' : '' }}
                                class="w-5 h-5 text-green-500 focus:ring-green-500 rounded border-gray-300">
                            <div class="flex flex-col">
    ...
    <script>
        ...
        class MyUploadAdapter {
            ...
            _initRequest() {
                ...
                xhr.open('POST', '{{ route('admin.berita.upload-image') }}', true);
    ...
    <script>
        ...
        // Restore existing value on page load
        var existingKategori = document.getElementById('kategori-input').value;
        if (existingKategori) {
            selectKategori(existingKategori);
        }
    </script>
@endsection