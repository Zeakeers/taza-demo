        ...
        <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8">
                ...
                @foreach($form_fields as $field)
                    @php $val = getFieldValue($volunteer, $field['name']); @endphp
                    
                    <div class="{{ $field['type'] == 'textarea' ? 'md:col-span-2' : '' }}">
                        <label class="block text-xs uppercase tracking-wider text-gray-400 font-bold mb-1">{{ $field['label'] }}</label>
                        
                        @if($field['name'] == 'no_hp' || $field['type'] == 'tel')
                        ...
                        @else
                            <p class="text-zinc-800 {{ $field['name'] == 'nama' ? 'font-medium text-lg' : '' }}">{{ $val ?: '-' }}</p>
                        @endif
                    </div>
                @endforeach
            </div>
