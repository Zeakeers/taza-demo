...
@section('header', 'Data Permohonan Bantuan')

@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-zinc-800">Daftar Permohonan Bantuan</h2>
        <p class="text-gray-500 text-sm mt-1">Kelola data permohonan bantuan dari website.</p>
    </div>
</div>
        ...
        <table class="w-full text-left whitespace-nowrap">
            <thead>
                <tr class="text-gray-400 text-xs uppercase tracking-widest border-b border-gray-50">
                    <th class="pb-4 font-bold">Nama Pemohon</th>
            ...
            <tbody class="text-sm">
                ...
                <tr class="border-b border-gray-50 hover:bg-gray-50 transition-colors group">
                    <td class="py-5 font-bold text-zinc-800">{{ $item->nama_pemohon }}</td>
                    ...
                    <td class="py-5 text-gray-500">Rp {{ number_format($item->nominal, 0, ',', '.') }}</td>
                    ...
                    <td class="py-5 text-right space-x-1 flex justify-end gap-1">
                        <a href="{{ route('admin.permohonan-bantuan.show', $item->id) }}" class="inline-flex items-center justify-center p-2 rounded-xl text-zinc-500 hover:bg-zinc-100 transition-all" title="Lihat Detail">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </a>
                        <a href="{{ route('admin.permohonan-bantuan.edit', $item->id) }}" class="inline-flex items-center justify-center p-2 rounded-xl text-blue-500 hover:bg-blue-50 transition-all" title="Edit">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </a>
                        <button type="button" onclick="openDeleteModal('delete-bantuan-{{ $item->id }}')" class="inline-flex items-center justify-center p-2 rounded-xl text-red-500 hover:bg-red-50 transition-all" title="Hapus">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                            </svg>
                        </button>
                        
                        <x-admin.delete-modal id="delete-bantuan-{{ $item->id }}" action="{{ route('admin.permohonan-bantuan.destroy', $item->id) }}" title="Hapus Data Permohonan" message="Apakah Anda yakin ingin menghapus data permohonan {{ $item->nama_pemohon }}? Tindakan ini tidak dapat dibatalkan." />
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="py-8 text-center text-gray-500">Belum ada data permohonan bantuan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
