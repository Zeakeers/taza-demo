...
@section('header', 'Detail Permohonan Bantuan')
...
<div class="mb-6 flex justify-between items-center">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h2 class="text-2xl font-bold text-zinc-800">Detail Permohonan</h2>
            <p class="text-gray-500 text-sm mt-1">Informasi lengkap permohonan bantuan.</p>
        </div>
    </div>
            ...
            <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
                <div x-show="showAccModal" x-transition.scale.origin.bottom class="bg-white rounded-[2rem] shadow-2xl w-full max-w-md relative overflow-hidden pointer-events-auto">
                    ...
                    <div class="p-8 text-center sm:p-10">
                        ...
                        <h3 class="text-2xl font-bold text-dark mb-3">Terima Permohonan</h3>
                        <p class="text-gray-500 mb-8">Anda yakin ingin menyetujui permohonan ini? Status akan diubah menjadi "Diterima".</p>
                        <div class="flex flex-col sm:flex-row gap-3 justify-center">
                            ...
                            <form action="{{ route('admin.permohonan-bantuan.update-status', $permohonanBantuan->id) }}" method="POST" class="w-full sm:w-auto m-0">
                                @csrf
            ...
            <div class="absolute inset-0 flex items-center justify-center p-4 pointer-events-none">
                <div x-show="showTolakModal" x-transition.scale.origin.bottom class="bg-white rounded-[2rem] shadow-2xl w-full max-w-md relative overflow-hidden pointer-events-auto">
                    ...
                    <div class="p-8 text-center sm:p-10">
                        ...
                        <h3 class="text-2xl font-bold text-dark mb-3">Tolak Permohonan</h3>
                        <p class="text-gray-500 mb-8">Anda yakin ingin menolak permohonan ini? Status akan diubah menjadi "Ditolak".</p>
                        <div class="flex flex-col sm:flex-row gap-3 justify-center">
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <div class="md:col-span-2 space-y-6">
        <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            <h3 class="text-lg font-bold text-zinc-800 mb-6 pb-4 border-b border-gray-100">Informasi Pemohon</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Nama Pemohon</p>
                    <p class="font-semibold text-zinc-800 text-lg">{{ $permohonanBantuan->nama_pemohon }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">No WhatsApp</p>
                    @php $wa = preg_replace('/^0/', '62', preg_replace('/[^0-9]/', '', $permohonanBantuan->no_whatsapp)); @endphp
                ...
                <div>
                    ...
                    <p class="font-semibold text-zinc-800 text-lg">{{ $permohonanBantuan->email ?: '-' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Jenis Pemohon</p>
                    <p class="font-semibold text-zinc-800 text-lg">{{ $permohonanBantuan->jenis_pemohon ?: '-' }}</p>
                </div>
                <div class="md:col-span-2">
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Alamat Domisili</p>
                    <p class="font-semibold text-zinc-800">{{ $permohonanBantuan->alamat_domisili }}</p>
                </div>
            </div>
        </div>

        <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            <h3 class="text-lg font-bold text-zinc-800 mb-6 pb-4 border-b border-gray-100">Detail Permohonan</h3>
            
            <div class="space-y-6">
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Deskripsi Permohonan / Alasan Mengajukan</p>
                    <div class="p-4 bg-gray-50 rounded-2xl text-zinc-700 mt-2">
                        {{ $permohonanBantuan->deskripsi ?: 'Tidak ada deskripsi' }}
                    </div>
                </div>
                
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Nominal yang Diajukan</p>
                    <p class="font-bold text-primary text-2xl">Rp {{ number_format($permohonanBantuan->nominal, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        
        <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            <h3 class="text-lg font-bold text-zinc-800 mb-6 pb-4 border-b border-gray-100">Informasi Tambahan</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Sumber Info</p>
                    <p class="font-semibold text-zinc-800">{{ $permohonanBantuan->sumber_info ?: '-' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Pernah Mengajukan Sebelumnya?</p>
                    <p class="font-semibold text-zinc-800">{{ $permohonanBantuan->pernah_mengajukan ?: '-' }}</p>
                </div>
                <div class="md:col-span-2">
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Referensi</p>
                    <p class="font-semibold text-zinc-800">{{ $permohonanBantuan->referensi ?: '-' }}</p>
                </div>
                <div class="md:col-span-2">
                    <p class="text-xs text-gray-400 font-medium uppercase tracking-wider mb-1">Waktu Terakhir Mengajukan Bantuan</p>
                    <p class="font-semibold text-zinc-800">{{ $permohonanBantuan->waktu_terakhir_mengajukan ?: '-' }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            <h3 class="text-lg font-bold text-zinc-800 mb-6 pb-4 border-b border-gray-100">Status Permohonan</h3>
            
            <div class="flex flex-col gap-4">
                <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-500">Status Saat Ini</span>
                    @if($permohonanBantuan->status == 'pending')
                    ...
                    @elseif($permohonanBantuan->status == 'acc')
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                    ...
                    <span class="font-semibold text-zinc-800">{{ $permohonanBantuan->created_at->format('d M Y, H:i') }}</span>
                </div>
            </div>
        </div>

        <div class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm">
            <h3 class="text-lg font-bold text-zinc-800 mb-6 pb-4 border-b border-gray-100">Lampiran KTP</h3>
            
            @if($permohonanBantuan->foto_ktp)
                <div class="rounded-2xl overflow-hidden border border-gray-200">
                    <img src="{{ asset('storage/' . $permohonanBantuan->foto_ktp) }}" alt="Foto KTP" class="w-full h-auto object-cover">
                </div>
                <a href="{{ asset('storage/' . $permohonanBantuan->foto_ktp) }}" target="_blank" class="mt-4 w-full flex justify-center items-center gap-2 py-3 bg-gray-50 hover:bg-gray-100 text-zinc-700 font-semibold rounded-xl transition-colors">
            ...
            @else
                <div class="py-10 text-center bg-gray-50 rounded-2xl border border-dashed border-gray-200 text-gray-400">
                    ...
                    <p class="text-sm font-medium">Tidak ada lampiran KTP</p>
                </div>
            @endif
        </div>
    </div>
