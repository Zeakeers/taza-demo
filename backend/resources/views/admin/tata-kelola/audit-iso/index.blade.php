...
@section('header', 'Kelola Audit ISO')
@section('content')
<div class="mb-6 flex justify-between items-center">
    <div>
        <h2 class="text-2xl font-bold text-gray-800">Daftar Audit ISO</h2>
    </div>
    <a href="{{ route('admin.tata-kelola.audit-iso.create') }}" class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
        + Tambah Baru
    </a>
</div>
                ...
                <tr class="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.tata-kelola.audit-iso.edit', $item->id) }}" class="p-2 bg-yellow-50 text-yellow-600 hover:bg-yellow-100 rounded-lg transition-colors">Edit</a>
                            <form action="{{ route('admin.tata-kelola.audit-iso.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?');">
                                @csrf
                                @method('DELETE')
