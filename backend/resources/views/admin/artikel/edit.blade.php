...
@section('content')
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-zinc-800">Edit Artikel</h2>
        <p class="text-gray-500 text-sm mt-1">Perbarui konten artikel yang sudah ada.</p>
    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-zinc-700 mb-2">Kategori <span
                                class="text-red-500">*</span></label>
                        <input type="hidden" name="kategori" id="kategori-input" value="{{ old('kategori', $artikel->kategori) }}" required>
                    ...
                    <div>
                        ...
                        <textarea id="konten-editor" name="konten" rows="12" placeholder="Tuliskan isi artikel di sini..."
                            class="w-full bg-gray-50 border border-gray-200 text-zinc-800 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all resize-y">{{ old('konten', $artikel->konten) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <label for="file-upload"
                            ...
                            <div class="space-y-1 text-center w-full py-6 {{ $artikel->thumbnail ? 'hidden' : '' }}"
                            ...
                            <img id="image-preview"
                                ...
                                class="w-full object-contain {{ $artikel->thumbnail ? '' : 'hidden' }}" />

                            <!-- Overlay untuk ganti gambar -->
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none {{ $artikel->thumbnail ? '' : 'hidden' }}"
                                id="hover-overlay">
                            ...
                            <input id="file-upload-overlay" name="thumbnail_overlay" type="file"
                                class="absolute inset-0 w-full h-full opacity-0 cursor-pointer {{ $artikel->thumbnail ? '' : 'hidden' }}"
                                accept="image/*"
                    ...
                    <div class="space-y-4">
                        <label
                            ...
                            <input type="checkbox" name="show_on_home" value="1" {{ old('show_on_home', $artikel->show_on_home) ? 'checked' : '' }}
                        ...
                        <label
                            ...
                            <input type="checkbox" name="is_editor_choice" value="1" {{ old('is_editor_choice', $artikel->is_editor_choice) ? 'checked' : '' }}
                            ...
                            <div class="flex flex-col">
                                ...
                                <span class="text-xs text-gray-500">Hanya 1 artikel yang akan menjadi Pilihan Editor</span>
                            </div>
                        </label>

                        <label
                            class="flex items-center gap-3 p-3 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-50 transition-colors">
                            <input type="checkbox" name="is_published" value="1" {{ old('is_published', $artikel->is_published) ? 'checked' : '' }}
                                class="w-5 h-5 text-green-500 focus:ring-green-500 rounded border-gray-300">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold text-zinc-800">Publish Artikel</span>
                                <span class="text-xs text-gray-500">Jika uncheck, artikel disembunyikan (Draft)</span>
                            </div>
                        </label>
                    </div>
